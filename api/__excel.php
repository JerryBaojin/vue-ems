...
$action="导出了排名表格";
$actor=$db->prepare($sql);
$actor->execute(array($user,$action,$time));


ExportExcel($datas['data']);
...
function ExportExcel($dataSource){
  ...
  foreach ($dataSource as $key => $value) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('C'.$index, $value['dirName'])
                ->setCellValue('D'.$index, $value['nickname'])
                ->setCellValue('E'.$index, $value['workUnit'])
                ->setCellValue('F'.$index, $value['role'])
...
//  $objWriter->save('php://output');
  $objWriter->save('download/'.NAME.".xls");
	echo 'download/'.NAME.".xls";
